...
def insert_date(db, cursor, records):
    ...
    print("Adding entry to update history: " +
          str(records) + " records on " + now)
...
def create_cache(conn, cursor):
    ...
    while spot != stop:
        ...
        cursor.execute("""INSERT INTO rsids VALUES(?, ?)""",
                       (list[spot], ",".join(dict["IdList"])))
        for y in dict["IdList"]:
            ...
            if newdict:
                cursor.execute(
                    """INSERT INTO pminfo VALUES (?, ?)""", (y, newdict[y]))
        time.sleep(5)
        spot = spot - 1
